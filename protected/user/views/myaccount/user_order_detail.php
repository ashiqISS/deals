...
//                        'filter' => CHtml::listData(Products::model()->findAll(), 'id', 'product_name'),
                                                    'value' => function($model) {
                                                            ...
                                                            return '<a href="' . Yii::app()->baseUrl . '/index.php/products/detail/name/' . $product_details->canonical_name . '" target="_blank">' . $product_details->product_name . '-' . $product_details->product_code . '</a>';
                                                    },
                                                    'type' => 'raw',
                                                ),
                                                array('name' => '',
//                        'filter' => CHtml::listData(Products::model()->findAll(), 'id', 'product_name'),
                                                    'value' => function($model) {
                                                            $product_details = Products::model()->findByPk($model->product_id);
                                                            $folder = Yii::app()->Upload->folderName(0, 1000, $product_details->id);
                                                            return '<img style="width:100px;" class="catz" src="' . Yii::app()->baseUrl . '/uploads/products/' . $folder . '/' . $product_details->id . '/main.' . $product_details->main_image . '"  />';
                                                    },
                                                    'type' => 'raw',
                                                ),
                                                array('name' => 'Merchant',
...
//                        'filter' => CHtml::listData(Products::model()->findAll(), 'id', 'product_name'),
                                                    'value' => function($data) {
                                                            $order_history = OrderHistory::model()->findByAttributes(array('order_id' => $data->order_id, 'product_id' => $data->product_id), array('order' => 'id desc'));
                                                            return OrderStatus::model()->findByPk($order_history->order_status)->title . ' ' . OrderStatus::model()->findByPk($order_history->order_status)->description;
                                                    },
                                                            'type' => 'raw',
                                                        ),
                                                    ),
